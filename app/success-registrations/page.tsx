...
export default function SuccessRegistrationsPage() {
    const [registrations, setRegistrations] = useState<SuccessRegistration[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchQuery, setSearchQuery] = useState('');
    const [allRegistrations, setAllRegistrations] = useState<SuccessRegistration[]>([]);

    useEffect(() => {
        fetchSuccessRegistrations();
    }, []);
    ...
    const fetchSuccessRegistrations = async () => {
        setLoading(true);
        try {
            const registrationsSnapshot = await getDocs(collection(db, 'successRegistrations'));
            const registrationsData = registrationsSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            })) as SuccessRegistration[];

            console.log('Total registrations fetched:', registrationsData.length);
            setAllRegistrations(registrationsData);
            setRegistrations(registrationsData);
        } catch (error) {
            console.error('Error fetching success registrations:', error);
    ...
    const handleExportData = () => {
        try {
            ...
            a.download = `successful-registrations-${date}.csv`;
        ...
        return (
            <div className="flex justify-center items-center min-h-[600px]">
                <div className="text-center">
                    ...
                    <p className="text-gray-500">Loading successful registrations...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="sm:flex sm:items-center mb-6">
                <div className="sm:flex-auto">
                    <h1 className="text-2xl font-semibold text-gray-900">Successful Registrations</h1>
                    <p className="mt-2 text-sm text-gray-700">
                        A list of all verified registrations and their ID status.
                    </p>
                </div>
                                    ...
                                    {registrations.length === 0 ? (
                                        <tr>
                                            <td colSpan={9} className="py-8 text-center text-sm text-gray-500">
                                                No successful registrations found
                                            </td>
                                        </tr>
                                    ) : (
